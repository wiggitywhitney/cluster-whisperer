...
- **A running GCP cluster is required for PRD work in this repo.** Run `kubectl cluster-info` first. If it fails or shows no server, halt and ask the user to provision one before continuing. Do NOT skip, defer, or design around the missing cluster. To provision: `./demo/cluster/setup.sh gcp` (no vals wrapper needed).
