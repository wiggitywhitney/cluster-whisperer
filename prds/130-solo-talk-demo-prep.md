...
| 24 | Kyverno webhook connectivity to the control plane may lag behind `wait_for_cluster_running` returning RUNNING | Even after the cluster is RUNNING, the API server may briefly fail admission webhook calls with "No agent available" — the network path to Kyverno's service endpoint has not fully re-established. The current `run_helm_step` retries once after detecting RECONCILING, but does not verify webhook connectivity before retrying. **Open question**: should `run_helm_step` add a webhook connectivity probe before retrying, or multiple retries with backoff? |
