// ABOUTME: Unit tests for the kyverno-cli-allowlist.yaml ClusterPolicy manifest.
// ABOUTME: Validates structure, CLI SA scoping, deny conditions, and critical security settings.
...
const MANIFEST_PATH = join(__dirname, "kyverno-cli-allowlist.yaml");
...
describe("kyverno-cli-allowlist.yaml ClusterPolicy", () => {
  ...
  it("has the correct name", () => {
    ...
    expect(metadata.name).toBe("cluster-whisperer-cli-resource-allowlist");
  });

  it("has a distinct name from the MCP SA policy (no conflict)", () => {
    const metadata = policy.metadata as Record<string, unknown>;
    expect(metadata.name).not.toBe("cluster-whisperer-resource-allowlist");
  });
  ...
  describe("spec", () => {
    ...
    describe("rule: require-approved-resources", () => {
      ...
      describe("match", () => {
        ...
        it("scopes to the cluster-whisperer-cli ServiceAccount (not User identity)", () => {
          const any = match().any as Record<string, unknown>[];
          const subjects = any[0].subjects as Record<string, string>[];
          expect(subjects).toHaveLength(1);
          expect(subjects[0].kind).toBe("ServiceAccount");
          expect(subjects[0].name).toBe("cluster-whisperer-cli");
          // namespace is required for SA kind
