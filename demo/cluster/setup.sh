...
# Contains infrastructure URLs only. Audience-facing env vars
# (CLUSTER_WHISPERER_AGENT, _TOOLS, _VECTOR_BACKEND) are set
# live on stage after each vote.

# Kubeconfig for cluster-whisperer (not exported to presenter's shell)
CLUSTER_WHISPERER_KUBECONFIG=${KUBECONFIG_PATH}

# Vector DB ingress URLs
CLUSTER_WHISPERER_CHROMA_URL=http://chroma.${BASE_DOMAIN}
CLUSTER_WHISPERER_QDRANT_URL=http://qdrant.${BASE_DOMAIN}
