...
- GKE cluster running (`./demo/cluster/setup.sh gcp`)
  - Kyverno is installed automatically by setup. Verify it's running:
    ```bash
    kubectl --kubeconfig ~/.kube/config-cluster-whisperer get pods -n kyverno
    ```
    Expected: four pods Running (`admission-controller`, `background-controller`, `cleanup-controller`, `reports-controller`).
...
Run the reset between rehearsal runs too.
